...
namespace Blogifier.Admin.Components
{
    public class EasyMdeWrapper
    {
        ...
        private IJSInProcessObjectReference _mdeWrapper;
        private IJSObjectReference _mdeInstance;

        public Func<Task> OnInsertFile  { get; set; }

        public EasyMdeWrapper(IJSRuntime jsRuntime)
        {
        ...
        public async Task LoadEditorAsync(bool isFullToolbar, ElementReference mdeReference, ElementReference inputFileContainer)
        {
            ...
            var wrapper = await GetWrapperAsync();
            ...
            var options = new EditorOptions
            {
                ToolbarItems = new []
                {
                    new ToolbarItem
                    {
                        Name = "heading",
                        Title = "Heading",
                        Icon = "<svg fill=\"currentColor\" width=\"16\" height=\"16\" viewBox=\"0 0 16 16\" xmlns=\"http://www.w3.org/2000/svg\"><path d=\"M3 2H5V7.25L11 7.25V2H13V14H11V8.75L5 8.75V14H3V2Z\" /></svg>",
                        Order = 1
                    },
                    new ToolbarItem
                    {
                    ...
                    new ToolbarItem
                    {
                        Name = "italic",
                        Title = "Italic",
                        Icon = "<svg fill=\"currentColor\" width=\"16\" height=\"16\" viewBox=\"0 0 16 16\" xmlns=\"http://www.w3.org/2000/svg\"><path d=\"M12 1V3H9.71429L8.28571 13H10V15H4V13H6.28571L7.71429 3H6V1H12Z\" /></svg>",
                        Order = 4
                    },
                    new ToolbarItem { Name = "|", Order = 5 },
                    new ToolbarItem
                    {
                        Name = "ordered-list",
                        Title = "Numbered List",
                    ...
                    new ToolbarItem
                    {
                        ...
                        Icon = "<svg fill=\"currentColor\" width=\"16\" height=\"16\" viewBox=\"0 0 16 16\" xmlns=\"http://www.w3.org/2000/svg\"><g clip-path=\"url(#clip0)\"><rect x=\"16\" y=\"7\" width=\"2\" height=\"16\" rx=\"1\" transform=\"rotate(90 16 7)\" /></g></svg>",
                        Order = 12
                    },
                    new ToolbarItem
                    {
                        Name = "clean-block",
                        Title = "Clean block",
                        Icon = "<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"18\" height=\"18\" fill=\"currentColor\" class=\"bi bi-eraser\" viewBox=\"0 0 16 16\"><path d=\"M8.086 2.207a2 2 0 0 1 2.828 0l3.879 3.879a2 2 0 0 1 0 2.828l-5.5 5.5A2 2 0 0 1 7.879 15H5.12a2 2 0 0 1-1.414-.586l-2.5-2.5a2 2 0 0 1 0-2.828l6.879-6.879zm2.121.707a1 1 0 0 0-1.414 0L4.16 7.547l5.293 5.293 4.633-4.633a1 1 0 0 0 0-1.414l-3.879-3.879zM8.746 13.547 3.453 8.254 1.914 9.793a1 1 0 0 0 0 1.414l2.5 2.5a1 1 0 0 0 .707.293H7.88a1 1 0 0 0 .707-.293l.16-.16z\"/></svg>",
                        Order = 13,
                        CanHide = true
                    },
                    new ToolbarItem { Name = "|", Order = 14, CanHide = true },
                    new ToolbarItem
                    {
                        Name = "link",
                        Title = "Create Link",
                    ...
                    new ToolbarItem
                    {
                        Name = "file",
                        Title = "Insert File",
                        Icon = "<svg fill=\"currentColor\" width=\"16\" height=\"16\" viewBox=\"0 0 16 16\" xmlns=\"http://www.w3.org/2000/svg\"><path d=\"M8 6.5a.5.5 0 0 1 .5.5v1.5H10a.5.5 0 0 1 0 1H8.5V11a.5.5 0 0 1-1 0V9.5H6a.5.5 0 0 1 0-1h1.5V7a.5.5 0 0 1 .5-.5z\"/><path d=\"M14 4.5V14a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V2a2 2 0 0 1 2-2h5.5L14 4.5zm-3 0A1.5 1.5 0 0 1 9.5 3V1H4a1 1 0 0 0-1 1v12a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1V4.5h-2z\"/></svg>",
                        Order = 16,
                        Callback = JsCallback.Create(() => OnInsertFile?.Invoke() ?? Task.CompletedTask)
                    },
                    new ToolbarItem
                    {
                        Name = "insertYoutube",
                        ...
                        Order = 17,
                        Callback = JsCallback.Create(() => wrapper.InvokeVoidAsync("insertYoutube", _mdeInstance))
                    },
                    new ToolbarItem { Name = "|", Order = 18 },
                    new ToolbarItem
                    {
                        Name = "preview",
            ...
            if(!isFullToolbar)
            {
                options.ToolbarItems = options.ToolbarItems.Where(i => !i.CanHide);
            }
        ...
        {
            if(_mdeInstance == null)
            {
                throw new System.InvalidOperationException();
            }

            var wrapper = await GetWrapperAsync();
            await wrapper.InvokeVoidAsync("setEditorValue", _mdeInstance, text);

            if(onTextChanged != null)
            {
                await wrapper.InvokeVoidAsync("onValueChanged", _mdeInstance, JsCallback.Create<string>(value => onTextChanged(value)));
            }
        }
        ...
        public async Task<string> GetEditorTextAsync()
        {
            ...
            var wrapper = await GetWrapperAsync();
            return await wrapper.InvokeAsync<string>("getEditorValue", _mdeInstance);
        }

        public async Task<string> ReplaceSelectionAsync(string value)
        {
            if(_mdeInstance == null)
            {
                throw new System.InvalidOperationException();
            ...
            var wrapper = await GetWrapperAsync();
            return await wrapper.InvokeAsync<string>("replaceSelection", _mdeInstance, value);
        }

        private async Task<IJSInProcessObjectReference> GetWrapperAsync()
        {
            if(_mdeWrapper == null)
            {
                _mdeWrapper = await _jsRuntime.InvokeAsync<IJSInProcessObjectReference>("import", "./admin/js/easyMdeWrapper.js");
            }
        ...
        public async ValueTask DisposeAsync()
        {
            if(_mdeInstance != null)
            {
                await _mdeInstance.DisposeAsync();
                _mdeInstance = null;
            }
            if(_mdeWrapper != null)
            {
                await _mdeWrapper.DisposeAsync();
                _mdeWrapper = null;
            }
        }
    }
